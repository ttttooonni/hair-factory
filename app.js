...
const styles = [
  {id:"taper-fade",category:"tendencia",title:"Taper Fade",description:"Transición suave en patillas y nuca, con un acabado limpio y natural.",tags:["Moderno","Versátil","Mantenimiento medio"],image:"./assets/styles/taper-fade.png"},
  {id:"textured-crop",category:"tendencia",title:"Textured Crop",description:"Parte superior texturizada y laterales limpios para un acabado actual.",tags:["Textura","Natural","Actual"],image:"./assets/styles/textured-crop.png"},
  {id:"buzz-cut",category:"tendencia",title:"Buzz Cut",description:"Corto, definido y práctico para quien busca sencillez.",tags:["Corto","Práctico","Bajo mantenimiento"],image:"./assets/styles/buzz-cut.png"},
  {id:"low-fade",category:"tendencia",title:"Low Fade",description:"Degradado bajo y discreto que conserva más peso en la parte superior.",tags:["Elegante","Discreto","Versátil"],image:"./assets/styles/low-fade.png"},
  {id:"clasico",category:"clasico",title:"Corte clásico",description:"Líneas limpias y proporción para un estilo atemporal.",tags:["Clásico","Elegante","Atemporal"],image:"./assets/styles/clasico.png"},
  {id:"side-part",category:"clasico",title:"Side Part",description:"Raya lateral definida, volumen controlado y acabado profesional.",tags:["Distinción","Profesional","Clásico"],image:"./assets/styles/side-part.png"},
  {id:"crew-cut",category:"clasico",title:"Crew Cut",description:"Corto, funcional y equilibrado, fácil de adaptar.",tags:["Limpio","Práctico","Versátil"],image:"./assets/styles/crew-cut.png"},
  {id:"tijera",category:"clasico",title:"Corte a tijera",description:"Trabajo de tijera para conservar movimiento, volumen y caída natural.",tags:["Natural","Movimiento","Tradicional"],image:"./assets/styles/tijera.png"},
  {id:"barba-3",category:"barba",title:"Barba de 3 días",description:"Barba corta y natural con líneas limpias y mantenimiento sencillo.",tags:["Natural","Fácil mantenimiento"],image:"./assets/styles/barba-3.png"},
  {id:"barba-corta",category:"barba",title:"Barba corta perfilada",description:"Contornos definidos y longitud corta para una imagen cuidada.",tags:["Limpia","Moderna","Definida"],image:"./assets/styles/barba-corta.png"},
  {id:"barba-media",category:"barba",title:"Barba media",description:"Longitud intermedia para trabajar forma, densidad y conexión con el corte.",tags:["Equilibrada","Versátil"],image:"./assets/styles/barba-media.png"},
  {id:"barba-larga",category:"barba",title:"Barba larga",description:"Mayor longitud y presencia, manteniendo forma y contornos.",tags:["Carácter","Presencia","Estilo propio"],image:"./assets/styles/barba-larga.png"}
];
...
function renderStyles(filter="tendencia"){
 ...
 $("#styleGrid").innerHTML=list.map(s=>`<article class="style-card" data-id="${s.id}"><img class="style-image" loading="lazy" src="${s.image}" alt="Referencia del estilo ${escapeHtml(s.title)}" onerror="this.onerror=null;this.src='./assets/logo.svg'"><div class="style-body"><span class="card-kicker">${s.category==="barba"?"BARBA":s.category==="clasico"?"CLÁSICO":"TENDENCIA"}</span><h3>${escapeHtml(s.title)}</h3><p>${escapeHtml(s.description)}</p><div class="tags">${s.tags.slice(0,2).map(t=>`<span>${escapeHtml(t)}</span>`).join("")}</div></div></article>`).join("");
 $$(".style-card").forEach(c=>c.addEventListener("click",()=>openStyle(c.dataset.id)));
}
function openStyle(id){const s=styles.find(x=>x.id===id);if(!s)return;$("#dialogImage").src=s.image; $("#dialogImage").onerror=()=>{$("#dialogImage").src="./assets/logo.svg";};$("#dialogImage").alt=`Referencia del estilo ${s.title}`;$("#dialogCategory").textContent=s.category==="barba"?"BARBA":s.category==="clasico"?"CLÁSICOS":"TENDENCIAS";$("#dialogTitle").textContent=s.title;$("#dialogDescription").textContent=s.description;$("#dialogTags").innerHTML=s.tags.map(t=>`<span>${escapeHtml(t)}</span>`).join("");$("#dialogBook").href=wa();$("#dialogDownload").href=s.image;$("#dialogDownload").download=`hair-factory-${s.id}.svg`;$("#styleDialog").showModal();}
